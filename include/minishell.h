...
/*   Updated: 2025/03/27 19:00:08 by ckrasniq         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// Command types
// Simple command (e.g., "ls -l")
// Pipeline (e.g., "ls | grep foo")
...
//	Return value either of key or a non-freeable empty string if not found
...
// get a token type representation for errors. If word,
// it returns value of token
...
/*
	...
	with "replace"
*/
void			ft_replace(char *content, char search,
					char replace, int truncate);
...
void			ms_sig_handler_interactive(int signal, siginfo_t *info,
					void *ctx);
...
void			process_quote_content(t_lexer *lexer, char *buffer,
					int *i, char quote_char);
void			process_variable_name(t_lexer *lexer, char *buffer,
					int *i, bool has_braces);
void			process_variable_in_word(t_lexer *lexer, char *buffer,
					int *i, int *in_quotes);
...
void			process_char(t_dyn_str *result, char *str, long *ctr,
					int *in_quotes);
...
int				handle_builtin_or_redirections(t_command *cmd, char **env,
					int *saved_fds);
...
void			ft_strncat(char *target, const char *source, size_t amount,
					size_t max);
